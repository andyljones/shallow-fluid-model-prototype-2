...
namespace Tests.SimulatorTests.ShallowFluidTests.MultigridSolverTests.InterpolatorTests
{
    ...
    {
        [Test]
        public void Interpolate_OnANodeWithOneNeighbourAtTheSameLocation_ShouldCopyTheNeighboursValue()
        {
            var interpolationGraph = new Graph<int> { { 0, new List<int> { 1 } } };
            var relativePositions = new VectorFieldMap<int>
            {
                {0, new VectorField<int> {{1, new Vector3(0, 0, 0)}}}
            };
            var fineGeometry = A.Fake<IGeometry<int>>();
            A.CallTo(() => fineGeometry.RelativePositions).Returns(relativePositions);
            var interpolator = new WeightedAverageInterpolator<int>(interpolationGraph, fineGeometry);
        ...
        {
            var interpolationGraph = new Graph<int> { { 0, new List<int> { 1, 2 } } };
            var relativePositions = new VectorFieldMap<int>
            {
                {0, new VectorField<int> {{1, new Vector3(1, 0, 0)}, {2, new Vector3(0, 1, 0)}}}
            };
            var geometry = A.Fake<IGeometry<int>>();
            A.CallTo(() => geometry.Graph).Returns(interpolationGraph);
            A.CallTo(() => geometry.RelativePositions).Returns(relativePositions);
            var interpolator = new WeightedAverageInterpolator<int>(interpolationGraph, geometry);
